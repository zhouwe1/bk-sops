...
class TaskResourceProvider(ResourceProvider):
    ...
    def list_instance(self, filter, page, **options):
        ...
        if not (filter.parent or filter.search or filter.resource_type_chain):
            queryset = TaskFlowInstance.objects.filter(is_deleted=Value(0), project__tenant_id=options["tenant_id"])
        elif filter.parent:
            ...
            queryset = TaskFlowInstance.objects.filter(task_filter, project_id__in=list(project_ids))
    ...
    def fetch_instance_info(self, filter, **options):
        ...
        queryset = TaskFlowInstance.objects.filter(id__in=ids, project__tenant_id=options["tenant_id"])
    ...
    def list_instance_by_policy(self, filter, page, **options):
        ...
        queryset = TaskFlowInstance.objects.filter(filters, project__tenant_id=options["tenant_id"])
        count = queryset.count()
        results = [
            {"id": str(task.id), "display_name": task.name} for task in queryset[page.slice_from : page.slice_to]
        ]
